...
static uint16_t score_for_elapsed(uint16_t elapsed_s) {
	...
	uint32_t score = (1000U + ((uint32_t)remaining * 100U)) * ((uint32_t)nb_game_settings_get_difficulty() + 1U);
...
view_dynamic_t dyn_view_item_game_score = {{.item_type = ITEM_TYPE_DYNAMIC}, view_scr_game_score};
