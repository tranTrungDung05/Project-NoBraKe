...
static void draw_track(void) {
	...
	for (uint8_t i = 1; i < NB_GAME_VISIBLE_LINES; ++i) {
		...
		view_render.drawLine(far_left, far_line->Y, near_left, near_line->Y, WHITE);
		view_render.drawLine(far_right, far_line->Y, near_right, near_line->Y, WHITE);

		if ((stripe_base + i) % 2 != 0) {
			view_render.drawLine(far_left - 3, far_line->Y, near_left - 3, near_line->Y, WHITE);
			view_render.drawLine(far_right + 3, far_line->Y, near_right + 3, near_line->Y, WHITE);
		}
...
static void draw_trees(void) {
	for (uint8_t i = 0; i < NB_GAME_TREE_COUNT; ++i) {
		...
		if (tree->size >= 20) {
			...
			view_render.drawTriangle(tree->x, tree->y - tree->size, tree->x - half, tree->y - tree->size / 3, tree->x + half, tree->y - tree->size / 3, WHITE);
		}
	}
}
...
static void draw_game() {
	...
	if (game_state == GAME_FINISHING) {
		draw_finish_gate();
		view_render.drawBitmap(car.x, car.y, bitmap_car_straight, NB_GAME_CAR_W, NB_GAME_CAR_H, WHITE);
	}
	else if (car.visible) {
		view_render.drawBitmap(car.x, car.y, bitmap_car_straight, NB_GAME_CAR_W, NB_GAME_CAR_H, WHITE);
		if (car.is_crash) {
...
view_dynamic_t dyn_view_item_nobrake_game = {{.item_type = ITEM_TYPE_DYNAMIC}, draw_game};
...
void scr_nobrake_game_handle(ak_msg_t *msg) {
	...
	case NB_GAME_TIME_TICK: {
		...
		if (car.is_crash || (nb_game_track_get_time_left_s() == 0 && track.pos < track.length)) {
